...
class PromiseTest
{
	public function assertTrue(expression :Bool, ?posInfos :haxe.PosInfos)
	{
		if (!expression) {
	...
	public function assertFalse(expression :Bool, ?posInfos :haxe.PosInfos)
	{
		assertTrue(!expression, posInfos);
		return !expression;
	}

	public function assertEquals<T>(val1 :T, val2 :T, ?posInfos :haxe.PosInfos)
	{
		if (val1 != val2) {
	...
	public function assertNotEquals(val1 :Dynamic, val2 :Dynamic, ?posInfos :haxe.PosInfos)
	{
		if (val1 == val2) {
			throw '${posInfos.className}.${posInfos.methodName}:${posInfos.lineNumber} Failed assertion: ${val1} == ${val2}';
		}
	...
	public function assertIsNull(val1 :Dynamic, ?posInfos :haxe.PosInfos)
	{
		if (val1 != null) {
	...
	public function assertNotNull(val1 :Dynamic, ?posInfos :haxe.PosInfos)
	{
		if (val1 == null) {
			throw '${posInfos.className}.${posInfos.methodName}:${posInfos.lineNumber} Failed assertion: ${val1} == null';
		}
